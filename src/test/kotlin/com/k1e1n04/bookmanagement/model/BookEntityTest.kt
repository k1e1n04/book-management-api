...
class BookEntityTest {
    ...
    fun `new throws exception for invalid author IDs`() {
        val title = "Test Book"
        val price = 1500
        val authorIds = listOf("invalid-uuid", AUTHOR_ID_2) // 無効なUUID
    ...
    fun `new throws exception for duplicate author IDs`() {
        val title = "Test Book"
        val price = 1500
        val authorIds = listOf(AUTHOR_ID_1, AUTHOR_ID_1) // 重複した著者ID
    ...
    fun `update throws exception for invalid author IDs`() {
        ...
        assertThatThrownBy {
            book.update(
                title = "New Title",
                price = 1200,
                authorIds = listOf("invalid-uuid", AUTHOR_ID_2), // 無効なUUID
                status = PublicationStatus.PUBLISHED
            )
        }.isInstanceOf(DomainValidationException::class.java)
            .hasMessageContaining("著者IDの形式が不正です。")
    ...
    fun `update throws exception for duplicate author IDs`() {
        ...
        assertThatThrownBy {
            book.update(
                title = "New Title",
                price = 1200,
                authorIds = listOf(AUTHOR_ID_1, AUTHOR_ID_1), // 重複した著者ID
                status = PublicationStatus.PUBLISHED
            )
        }.isInstanceOf(DomainValidationException::class.java)
            .hasMessageContaining("書籍の著者IDは重複してはいけません。")
